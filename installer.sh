...
if [ "$PACMANCONF_ADD_ANS" == "y" ] || [ "$PACMANCONF_ADD_ANS" == "Y" ]; then
	...
	sudo curl -s -o /etc/pacman.conf https://githubusercontent.com/virtualdemon/archlinux-i3wm/master/etc/pacman.conf
...
if [ "$INST_TOR" == "y" ] || [ "$INST_TOR" == "Y" ];then
	...
	sudo curl -s -o /etc/tor/torrc https://githubusercontent.com/virtualdemon/archlinux-i3wm/master/etc/tor/torrc
...
if [ ! -z $(pacman -Qq lightdm-gtk-greeter) ];then
	sudo curl -s -o /etc/lightdm/lightdm-gtk-greeter.conf https://githubusercontent.com/virtualdemon/archlinux-i3wm/master/etc/lightdm/lightdm-gtk-greeter.conf
	sudo mkdir -p /usr/share/lightdm-background
	sudo curl -s -o /usr/share/lightdm-background/08303b0034dec9b5a48e98c305464edd.JPEG https://githubusercontent.com/virtualdemon/archlinux-i3wm/master/backgrounds/08303b0034dec9b5a48e98c305464edd.JPEG
fi
...
for BLOCKLET in ${BLOCKLETS[@]} ;do
	...
	curl -s -o $HOME/.config/i3/blocklets/$BLOCKLET https://githubusercontent.com/virtualdemon/archlinux-i3wm/master/i3-config-files/blocklets/$BLOCKLET
done
...
for BIN_SCRIPT in ${BIN_SCRIPTS[@]}; do
	...
	curl -s -o $HOME/bin/$BIN_SCRIPT https://githubusercontent.com/virtualdemon/archlinux-i3wm/master/bin/$BIN_SCRIPT
done
...
for I3_CONFIG in ${I3_CONFIGS[@]};do
	...
	curl -s -o $HOME/.config/i3/$I3_CONFIG https://githubusercontent.com/virtualdemon/archlinux-i3wm/master/i3-config-files/$I3_CONFIG
done

clear
for HOME_DOTFILE in ${HOME_DOTFILES[@]};do
	echo "Adding $HOME_DOTFILE"
	curl -s -o $HOME/.$HOME_DOTFILE https://githubusercontent.com/virtualdemon/archlinux-i3wm/master/dotfiles/$HOME_DOTFILE
done

clear
for DOTFILE in ${DOTFILES[@]}; do
	echo "Adding $DOTFILE"
	curl -s -o $HOME/.config/$DOTFILE https://githubusercontent.com/virtualdemon/archlinux-i3wm/master/dotfiles/$DOTFILE
done

for FONT in ${FONTS[@]}; do
	echo "Adding $FONT"
	curl -s -o $HOME/.fonts/$FONT https://githubusercontent.com/virtualdemon/archlinux-i3wm/master/fonts/$FONT
done
